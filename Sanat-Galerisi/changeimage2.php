...
if (strlen($_SESSION['agmsaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!in_array($extension, $izin_verilen_uzantilar)) {
            echo "<script>alert('Geçersiz format. Sadece jpg / jpeg / png / gif formatına izin verilir');</script>";
        } else {
            $sanat_resmi = md5($appic) . $extension;
            move_uploaded_file($_FILES["images"]["tmp_name"], "images/" . $sanat_resmi);

            $sorgu = mysqli_query($con, "UPDATE tblartproduct SET Image2 ='$sanat_resmi' WHERE ID='$cid'");
