...
if (strlen($_SESSION['agmsaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!in_array($uzanti, $izin_verilen_uzantilar)) {
            echo "<script>alert('Geçersiz formatta. Sadece jpg / jpeg / png / gif formatına izin verilir');</script>";
        } else {
            $sanat_resmi = md5($appic) . $uzanti;
            move_uploaded_file($_FILES["images"]["tmp_name"], "images/" . $sanat_resmi);

            $sorgu = mysqli_query($con, "UPDATE tblartproduct SET Image ='$sanat_resmi' WHERE ID='$cid'");
